...
% 加载数据
Xori = xlsread('data/feature_1104_124x55'); 
Yori = xlsread('data/label.xls'); 
N = size(Xori, 1);
feature_nums = size(Xori, 2);
...
for seed_id = 1:seed_nums
    ...
    tset_nums = size(tset_array,2);
    disp('====================================')
    disp(['Seed is ', num2str(seed)]);
    for t_idx =1:tset_nums

%         Tset = cell2mat(tset_array(t_idx));
        Tset = tset_array{t_idx};
    
        sum_rmse_all = 0;
        sum_rmse_2 = 0;
        
        
%         disp(['Tset is ', num2str(Tset)]);

        Xo = Xori(randomIndex, :);  %124x53 列都被保持）
        Y = Yori(randomIndex, :);
        ...
        for random = 1:train_times
            ...
            disp(['selected features ', num2str(Tset),' result is ' num2str(RMSE_2)]);
        ...
        disp(['Average selected features ', num2str(Tset), ' result is ' num2str(sum_rmse_2 / train_times)]);
    end
